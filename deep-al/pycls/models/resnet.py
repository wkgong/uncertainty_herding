...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor, y: Tensor = None) -> Tensor:
        ...
        if y is None:
            ...
            output_dict = {
                'preds': x, 'features': z, 'labels': y, 'onehot_labels': y_onehot.float()}
        return output_dict


    def forward_classifier(self, embeddings: Tensor, y: Tensor = None) -> Tensor:
        z = embeddings
        if self.use_dropout:
            z = self.drop(z)
    ...
    def forward(self, x: Tensor, y: Tensor = None) -> Tensor:
        if self.last_layer == 'fc':
            ...
            raise NotImplementedError(f'last_layer: {self.last_layer} not implemented')
