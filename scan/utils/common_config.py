...
def get_model(p, pretrain_path=None):
    # Get backbone
    if p['backbone'] == 'resnet18':
        if p['train_db_name'] in ['cifar-10', 'cifar-10-00', 'cifar-10-01', 'cifar-10-02', 'cifar-10-05', 'cifar-100', 'imbal-cifar-10', 'imbal-cifar-100']:
            ...
            backbone = resnet18()
        elif p['train_db_name'] in ['tiny-imagenet', 'imbal-tiny-imagenet', 'imbal-place-365', 'domainnet']:
...
def get_train_dataset(p, transform, to_augmented_dataset=False,
    ...
    if p['train_db_name'] == 'cifar-10':
    ...
    elif p['train_db_name'] == 'cifar-10-00':
        from data.cifar import CIFAR10
        dataset = CIFAR10(train=True, transform=transform, download=True, ratio=0.0)
        
    elif p['train_db_name'] == 'cifar-10-01':
        from data.cifar import CIFAR10
        dataset = CIFAR10(train=True, transform=transform, download=True, ratio=0.1)
    
    elif p['train_db_name'] == 'cifar-10-02':
        from data.cifar import CIFAR10
        dataset = CIFAR10(train=True, transform=transform, download=True, ratio=0.2)
        
    elif p['train_db_name'] == 'cifar-10-05':
        from data.cifar import CIFAR10
        dataset = CIFAR10(train=True, transform=transform, download=True, ratio=0.5)

    elif p['train_db_name'] == 'imbal-cifar-10':
        from data.cifar import IMBALANCECIFAR10
        dataset = IMBALANCECIFAR10(train=True, transform=transform, download=True)
    ...
    elif p['train_db_name'] == 'imbal-cifar-100':
        from data.cifar import IMBALANCECIFAR100
        dataset = IMBALANCECIFAR100(train=True, transform=transform, download=True)
                                    #, imbalance_ratio=0.2)
        ...
        dataset = TinyImageNet(root='./datasets', split='train', download=True, transform=transform)

    elif p['train_db_name'] == 'imbal-tiny-imagenet':
        from data.tinyimagenet import ImbalancedTinyImageNet
        dataset = ImbalancedTinyImageNet(root='./datasets', split='train', download=True,
                                         transform=transform) #, imbalance_ratio=0.2)
    ...
    elif p['train_db_name'] == 'imbal-place-365':
        from data.places import Places_LT
        dataset = Places_LT(train=True, transform=transform)

    elif p['train_db_name'] == 'domainnet':
        from data.domainnet import DomainNet
        dataset = DomainNet(root='./datasets', split='train', transform=transform)
...
def get_val_dataset(p, transform=None, to_neighbors_dataset=False):
    ...
    if p['val_db_name'] == 'cifar-10':
        ...
        from data.cifar import CIFAR10
        dataset = CIFAR10(train=False, transform=transform, download=True, ratio=1.0)
    ...
    elif p['val_db_name'] == 'imbal-cifar-10':
        from data.cifar import IMBALANCECIFAR10
        dataset = IMBALANCECIFAR10(train=False, transform=transform, download=True)

    elif p['val_db_name'] == 'cifar-100':
        from data.cifar import CIFAR100
        dataset = CIFAR100(train=False, transform=transform, download=True)

    elif p['val_db_name'] == 'imbal-cifar-100':
        from data.cifar import IMBALANCECIFAR100
        dataset = IMBALANCECIFAR100(train=False, transform=transform, download=True)
        ...
        from data.tinyimagenet import TinyImageNet
        dataset = TinyImageNet(root='./datasets', split='val', download=True, transform=transform)

    elif p['train_db_name'] == 'imbal-tiny-imagenet':
        from data.tinyimagenet import ImbalancedTinyImageNet
        dataset = ImbalancedTinyImageNet(root='./datasets', split='val', download=True, transform=transform)
    ...
    elif p['val_db_name'] == 'imbal-place-365':
        from data.places import Places_LT
        dataset = Places_LT(train=False, transform=transform)

    elif p['val_db_name'] == 'domainnet':
        from data.domainnet import DomainNet
        dataset = DomainNet(root='./datasets', split='test', transform=transform)
