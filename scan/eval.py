...
from utils.common_config import get_val_dataset, get_val_transformations, get_val_dataloader,\
                                get_model, get_train_transformations, get_train_dataset, get_train_dataloader
...
def main():
    ...
    train_transformations = get_train_transformations(config)
    train_dataset = get_train_dataset(config, train_transformations, split='train')
    ...
    if config['setup'] in ['simclr', 'moco']:
        ...
        train_predictions, train_features = get_predictions(config, train_dataloader, model,
                                                            return_features=True)
        print(clustering_stats)
        train_save_path = f"./results/{config['train_db_name']}/scan/features_seed{args.seed}.npy"
        test_save_path = f"./results/{config['val_db_name']}/scan/test_features_seed{args.seed}.npy"
        np.save(train_save_path, train_features.cpu().numpy())
        np.save(test_save_path, features.cpu().numpy())
