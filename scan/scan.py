...
def main():
    ...
    if os.path.exists(p['scan_checkpoint']):
        ...
        start_epoch = p['epochs']
    elif os.path.exists(p['scan_model']):
        print(colored('Restart from checkpoint {}'.format(p['scan_model']), 'blue'))
        checkpoint = torch.load(p['scan_model'], map_location='cpu')
        model.module.load_state_dict(checkpoint['model'])
        start_epoch = p['epochs']
    else:
    ...
    # save features
    val_dataset = get_val_dataset(p, val_transformations, to_neighbors_dataset = True)
    test_preds, test_features = get_predictions(p, val_dataloader, model, return_features=True)

    savedir = os.path.dirname(p['scan_checkpoint'])
    test_features_path = os.path.join(savedir, f'test_features_seed{args.seed}_clusters{args.num_clusters}.npy')

    np.save(test_features_path, test_features.numpy())
